...
from lib.common.util import sh, home
...
def add_fd_parser(parent: argparse.ArgumentParser) -> None:
    ...
    cmd_parser_choose = subparsers.add_parser("choose", help="Use FZF to choose a directory from the registry")
    cmd_parser_choose.add_argument("query", default="", nargs='?')
    cmd_parser_choose.set_defaults(func=cmd_fd_choose)

    cmd_parser_add = subparsers.add_parser("add", help="Add current working directory to registry")
    ...
    cmd_parser_edit = subparsers.add_parser("edit", help="Open the registry file in an editor")
