...
# Based on the pure prompt by Sindre Sorhus
...
GIT_COLOR_CLEAN=green
GIT_COLOR_DIRTY=red
GIT_COLOR_PENDING=gray
...
prompt_pure_preprompt_render() {
	...
	preprompt_parts+=('$prompt_username') # Username
    preprompt_parts+='%F{white}at%f'      # Separator
	preprompt_parts+=('$prompt_machine')  # Machine
	preprompt_parts+='%F{white}in%f'      # Separator
    preprompt_parts+='%F{13}%~%f'         # Path

	# Add git branch and dirty status info.
	typeset -gA prompt_pure_vcs_info
	if [[ -n $prompt_pure_vcs_info[branch] ]]; then
	    preprompt_parts+=('%F{white}on%f')
...
prompt_pure_setup() {
	...
	prompt_username='%F{9}%n%f'
	prompt_machine='%F{yellow}%m%f'
